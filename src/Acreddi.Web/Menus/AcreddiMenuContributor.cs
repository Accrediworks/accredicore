...
namespace Acreddi.Web.Menus;

public class AcreddiMenuContributor : IMenuContributor
{
    private readonly IConfiguration _configuration;

    public AcreddiMenuContributor(IConfiguration configuration)
    {
    ...
    {
        var l = context.GetLocalizer<AcreddiResource>();
        //Home
        ...
        context.Menu.AddItem(
            new ApplicationMenuItem(
                AcreddiMenus.HostDashboard,
                l["Menu:Dashboard"],
                "~/HostDashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(AcreddiPermissions.Dashboard.Host)
        );

        //Tenant Dashboard
        context.Menu.AddItem(
            new ApplicationMenuItem(
                AcreddiMenus.TenantDashboard,
                l["Menu:Dashboard"],
                "~/Dashboard",
                icon: "fa fa-line-chart",
                order: 2
            ).RequirePermissions(AcreddiPermissions.Dashboard.Tenant)
        );
        ...
        context.Menu.AddItem(
                ...
                new ApplicationMenuItem(
                    ...
                    l["Menu:Books"],
                    url: "/Books"
            ).RequirePermissions(AcreddiPermissions.Books.Default) 
        )
    );
