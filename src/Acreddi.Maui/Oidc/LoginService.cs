...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(AcreddiConsts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(AcreddiConsts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(AcreddiConsts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(AcreddiConsts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(AcreddiConsts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(AcreddiConsts.OidcConsts.RefreshTokenKeyName);
    }
}