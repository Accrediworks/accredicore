...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Accredi") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Accredi", DisplayName = "Accredi API", Resources = { "Accredi" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["Accredi_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["Accredi_Web:RootUrl"]!.EnsureEndsWith('/');

            /* Accredi_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Accredi_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        var consoleAndAngularClientId = configurationSection["Accredi_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["Accredi_App:RootUrl"]?.TrimEnd('/');
            await CreateApplicationAsync(
                applicationType: OpenIddictConstants.ApplicationTypes.Web,
        ...
        //MAUI Client
        var mauiClientId = configurationSection["Accredi_Maui:ClientId"];
        if (!mauiClientId.IsNullOrWhiteSpace())
        {
            var mauiClientRootUrl = configurationSection["Accredi_Maui:RootUrl"]?.Replace("_", "-");
            var mauiClientPostLogoutRedirectUri = configurationSection["Accredi_Maui:PostLogoutRedirectUri"]?.Replace("_", "-");
            await CreateApplicationAsync(
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Accredi_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Accredi_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        var webPublicTieredClientId = configurationSection["Accredi_Web_Public:ClientId"];
        if (!webPublicTieredClientId.IsNullOrWhiteSpace())
        {
            var webPublicTieredRootUrl = configurationSection["Accredi_Web_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Accredi_Web_Public:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
