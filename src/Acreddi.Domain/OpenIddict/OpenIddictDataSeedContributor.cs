...
public class OpenIddictDataSeedContributor : IDataSeedContributor, ITransientDependency
{
    ...
    {
        if (await _openIddictScopeRepository.FindByNameAsync("Acreddi") == null)
        {
            await _scopeManager.CreateAsync(new OpenIddictScopeDescriptor {
                Name = "Acreddi", DisplayName = "Acreddi API", Resources = { "Acreddi" }
            });
        }
    }
    ...
    private async Task CreateApplicationsAsync()
    {
        ...
        //Web Client
        var webClientId = configurationSection["Acreddi_Web:ClientId"];
        if (!webClientId.IsNullOrWhiteSpace())
        {
            var webClientRootUrl = configurationSection["Acreddi_Web:RootUrl"]!.EnsureEndsWith('/');

            /* Acreddi_Web client is only needed if you created a tiered
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Acreddi_Web:ClientSecret"] ?? "1q2w3e*",
        ...
        var consoleAndAngularClientId = configurationSection["Acreddi_App:ClientId"];
        if (!consoleAndAngularClientId.IsNullOrWhiteSpace())
        {
            var consoleAndAngularClientRootUrl = configurationSection["Acreddi_App:RootUrl"]?.TrimEnd('/');
            await CreateApplicationAsync(
                applicationType: OpenIddictConstants.ApplicationTypes.Web,
        ...
        //MAUI Client
        var mauiClientId = configurationSection["Acreddi_Maui:ClientId"];
        if (!mauiClientId.IsNullOrWhiteSpace())
        {
            var mauiClientRootUrl = configurationSection["Acreddi_Maui:RootUrl"]?.Replace("_", "-");
            var mauiClientPostLogoutRedirectUri = configurationSection["Acreddi_Maui:PostLogoutRedirectUri"]?.Replace("_", "-");
            await CreateApplicationAsync(
        ...
        // Swagger Client
        var swaggerClientId = configurationSection["Acreddi_Swagger:ClientId"];
        if (!swaggerClientId.IsNullOrWhiteSpace())
        {
            var swaggerRootUrl = configurationSection["Acreddi_Swagger:RootUrl"]?.TrimEnd('/');
        ...
        var webPublicTieredClientId = configurationSection["Acreddi_Web_Public:ClientId"];
        if (!webPublicTieredClientId.IsNullOrWhiteSpace())
        {
            var webPublicTieredRootUrl = configurationSection["Acreddi_Web_Public:RootUrl"]!.EnsureEndsWith('/');
            ...
            await CreateApplicationAsync(
                ...
                secret: configurationSection["Acreddi_Web_Public:ClientSecret"] ?? "1q2w3e*",
                grantTypes: new List<string> //Hybrid flow
                {
                    OpenIddictConstants.GrantTypes.AuthorizationCode, OpenIddictConstants.GrantTypes.Implicit
                },
