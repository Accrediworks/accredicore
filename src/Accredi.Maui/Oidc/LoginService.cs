...
public class LoginService : ILoginService, ITransientDependency
{
    ...
    {
        var token = await _storage.GetAsync(AccrediConsts.OidcConsts.AccessTokenKeyName);
    ...
    {
        var refreshToken = await _storage.GetAsync(AccrediConsts.OidcConsts.RefreshTokenKeyName);
    ...
    {
        await _storage.SetAsync(AccrediConsts.OidcConsts.AccessTokenKeyName, accessToken);
        await _storage.SetAsync(AccrediConsts.OidcConsts.RefreshTokenKeyName, refreshToken);
    }

    private async Task ClearTokenCacheAsync()
    {
        await _storage.RemoveAsync(AccrediConsts.OidcConsts.AccessTokenKeyName);
        await _storage.RemoveAsync(AccrediConsts.OidcConsts.RefreshTokenKeyName);
    }
}