...
namespace Accredi.Crm.ContactTelephones
{
    ...
    public class ContactTelephonesAppService : CrmAppService, IContactTelephonesAppService
    {
        ...
        {
            var totalCount = await _contactTelephoneRepository.GetCountAsync(input.FilterText, input.PhoneNumber, input.Type);
            var items = await _contactTelephoneRepository.GetListAsync(input.FilterText, input.PhoneNumber, input.Type, input.Sorting, input.MaxResultCount, input.SkipCount);
